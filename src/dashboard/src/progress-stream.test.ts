...
import { RECONNECT_BASE_MS, RECONNECT_MAX_MS, backoffMs, subscribeProgress } from "./api";
...
describe("reconnecting", () => {
  it("retries after the stream drops, without being asked", async () => {
    ...
    const stop = subscribeProgress(progress, (message) => errors.push(message));
    await settle();
  ...
  it("treats a clean close as a drop too", async () => {
    ...
    const stop = subscribeProgress(vi.fn(), (message) => errors.push(message));
    await settle();
  ...
  it("keeps exactly one connection open across a reconnect", async () => {
    ...
    const stop = subscribeProgress(vi.fn(), vi.fn());
    await settle();
  ...
  it("recovers when the server comes back", async () => {
    const { stub, attempts, failures } = stubFetch();
    const progress = vi.fn();
    const stop = subscribeProgress(progress, vi.fn());
    await settle();
