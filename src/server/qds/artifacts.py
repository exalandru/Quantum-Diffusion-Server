...
def default_cache_root() -> Path:
    ...
    Derived rather than written down as a path. The menubar app tells every child
    process where its data lives by handing it `QDS_SERVER_CONFIG`, which is a
    file *inside* that directory — so the directory holding the configuration is
    the application's data directory, whatever the app resolved to on this
    machine and for this user. That covers the packaged app, a development
    checkout pointed at its own config, and two accounts on one Mac, without any
    of them agreeing on a constant.

    The fallback is for a bare `qds serve` with nothing configured: the same
    location the app would have used, composed from the home directory and the
    bundle identifier rather than assumed.
    """
    configured = env.get("CONFIG")
    if configured:
