"""Chargement et validation de `server.config`.
...
DEFAULT_CONFIG_PATH = Path(__file__).resolve().parent.parent / "server.config"
...
def config_path() -> Path:
    return Path(os.environ.get(f"{ENV_PREFIX}CONFIG", DEFAULT_CONFIG_PATH))


def load_settings(path: Path | None = None) -> Settings:
    """Lit `server.config`, applique les variables d'environnement, valide.

    Une config absente n'est pas une erreur : les défauts du catalogue
    suffisent à démarrer.
    """
